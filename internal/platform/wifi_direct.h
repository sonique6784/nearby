...
class WifiDirectServerSocket final {
 public:
  ...
  // Returns ip address of the server socket
  std::string GetIPAddress() const {
    CHECK(impl_);
    return impl_->GetIPAddress();
  }

  // Returns portnumber of the server socket
  int GetPort() const {
    CHECK(impl_);
...
class WifiDirectMedium {
 public:
  ...
  bool StartWifiDirect() {
    ...
    return impl_->StartWifiDirect();
  }
  bool StopWifiDirect() { return impl_->StopWifiDirect(); }

  bool ConnectWifiDirect(const std::string& ssid,
                          const std::string& password) {
    MutexLock lock(&mutex_);
    wifidirect_credentials_.SetSSID(ssid);
    wifidirect_credentials_.SetPassword(password);
    return impl_->ConnectWifiDirect(&wifidirect_credentials_);
  }
  bool DisconnectWifiDirect() { return impl_->DisconnectWifiDirect(); }

  HotspotCredentials* GetCredential() {
    return &wifidirect_credentials_;
  }
 ...
 private:
  ...
  HotspotCredentials wifidirect_credentials_ ABSL_GUARDED_BY(mutex_);
};
